...
class SearchTools:
    ...
    def _fuzzy_match(
        self, query: str, text: str, threshold: float = 0.3
    ) -> Tuple[bool, float]:
        """
        模糊匹配函数
    ...
    def search_related_news_history(
        ...
        try:
            ...
            threshold = validate_threshold(
                threshold, default=0.4, min_value=0.0, max_value=1.0
            )
            limit = validate_limit(limit, default=50)
            ...
            while current_date <= search_end:
                try:
                    ...
                    all_titles, id_to_name, _ = (
                        self.data_service.parser.read_all_titles_for_date(current_date)
                    )
                    ...
                    for platform_id, titles in all_titles.items():
                        ...
                        for title, info in titles.items():
                            ...
                            combined_score = (
                                keyword_overlap * 0.7 + title_similarity * 0.3
                            )
                    ...
                    # 记录错误但继续处理其他日期
                    print(
                        f"Warning: 处理日期 {current_date.strftime('%Y-%m-%d')} 时出错: {e}"
                    )
            ...
            platform_distribution = Counter(
                [news["platform"] for news in all_related_news]
            )
    ...
    def find_related_news_unified(
        ...
        try:
            ...
            reference_title = validate_keyword(reference_title)
            threshold = validate_threshold(
                threshold, default=0.5, min_value=0.0, max_value=1.0
            )
            limit = validate_limit(limit, default=50)
            ...
            for search_date in search_dates:
                try:
                    all_titles, id_to_name, _ = (
                        self.data_service.parser.read_all_titles_for_date(search_date)
                    )
                    ...
                    for platform_id, titles in all_titles.items():
                        ...
                        for title, info in titles.items():
                            ...
                            if reference_keywords:
                                ...
                                # 混合相似度：70% 文本 + 30% 关键词
                                similarity = (
                                    0.7 * text_similarity + 0.3 * keyword_similarity
                                )
                            else:
            ...
            try:
                ...
                for feed_id, items in all_titles.items():
                    ...
                    for title, info in items.items():
                        ...
                        summary_match = (
                            query_lower in summary.lower() if summary else False
                        )
